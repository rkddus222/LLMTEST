...
def evaluate_user_question(user_question: str) -> str:
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            SystemMessage(
                content=load_prompt("LLMTEST/backend/prompts/question_evaluation/main_v1.prompt")
            ),
            (
                "human",
...
def simple_conversation(user_question: str) -> str:
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            SystemMessage(
                content=load_prompt("LLMTEST/backend/prompts/general_conversation/main_v1.prompt")
            ),
            (
                "human",
                "{user_question}",
...
def analyze_user_question(user_question: str) -> str:
    ...
    ANALYZE_PROMPT = ChatPromptTemplate.from_messages(
        [
            SystemMessage(
                content=load_prompt("LLMTEST/backend/prompts/question_analysis/main_v1.prompt")
            ),
            (
                "human",
...
def refine_user_question(user_question: str, user_question_analyze: str) -> str:
    ...
    REFINE_PROMPT = ChatPromptTemplate.from_messages(
        [
            SystemMessage(
                content=load_prompt("LLMTEST/backend/prompts/question_refinement/main_v1.prompt")
            ),
            (
                "human",
                """사용자 질문: 
...
def clarify_user_question(
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            SystemMessage(
                content=load_prompt("LLMTEST/backend/prompts/additional_question/main_v1.prompt")
            ),
            (
                "human",
                "원래 사용자 질문:\n{user_question}\n\n초기 질문 분석:\n{user_question_analyze}\n\n이전 질문 기록:\n{collected_questions}\n\n"
                + load_prompt("LLMTEST/backend/prompts/additional_question/human_postfix_v1.prompt"),
            ),
        ]
    )
...
def create_query(
    ...
    try:
        if flow_status == "KEEP":
            ...
            print("today:", today)
            # 프롬프트 로드 및 구성
            prefix = load_prompt("LLMTEST/backend/prompts/query_creation/prefix_v1.prompt").format(
                user_question=user_question,
                user_question_analyze=user_question_analyze,
                today=today,
                context="",
...
def business_conversation(user_question, sql_query, query_result) -> str:
    ...
    instruction = load_prompt("LLMTEST/backend/prompts/sql_conversation/main_v1.prompt").format(
